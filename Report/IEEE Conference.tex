...
\title{Entity Co-Location Applications}
...
\IEEEauthorblockA{LM2S - CNRS Joint Research Unit \\
University of Technology\\ Troyes, France - 10004\\
...
\begin{abstract}
%\boldmath
We present a methodology to develop novel applications that utilise the current state of the art in deep learning for object detection. Building upong the unified detection and localisation work in YOLO framework, we develop a semantic graph of objects present in any image. We use this graph for visualising the interaction pattern of objects and develop a semantic content based image retrieval system. We provide a proof of concept which can be further extended to automatically find anomalies encoutered during video surveillance tasks or to build recommender systems based on interest profile of users on the social media based on the entities in the images that they share. The code can be found at:
...
Humans have a natural ability to identify the objects from the images at a semantic level in a very robust way: invariant to illumination, pose and other geometric transforms. On identifying these objects, we humans sub-consciously start relating stuff to each other based on their pattern of co-location or co-occurence. This enables us to take a broader view of the subject and respond accordingly when confronted with task such as categorsing a scene in general or identifying pecularities in the way things are associated with each other. It is  a long standing research objective to develop mathematical models that can govern the machines to do the same. Current state of the art in deep learning community have shown significant progress along this line to identify various objects present in the image. In this paper we propose applications that can harness this input from machine learning and can be utilised for various tasks under a same framework. Our primary contributions involve:
    \begin{enumerate}
        ...
        \item Building Entity co-location graphs in a scalable Map-reduce compatible technique such that it can be scaled efficiently.
        ...
        \item Semantic content based image retrieval system using the extracted entity information.
    \end{enumerate}
...
\section{Methods}
Our proposed system can bebroadly divided in 3 parts: 
    \begin{enumerate}
    ...
    \subsection{The Entity Tagger}
        ...
        The network design of it's framework is inspired by the GoogLeNet model for image classification. It is first pre-trained on the ImageNet and then then the entire model with additional four convolutional layers on top is fine tuned by training it on the PASCAL VOC 2012 dataset which consists of 20 entity categories. It divides the entire image in S*S grid and predicts B bounding boxes for each section. Each bounding box contains 5 items: x,y, width, height and IoU conficence. The centre co-ordinates(x,y) are normalised in the range of 0-1 relative to the section, whereas the width and height are normalised in the range of 0-1 relative to the width and height of the input image. It also predicts the class probablities for C classes in each section. This enables it to predict the classes of the objects and their respective locations in the image simultaneously.
        ...
        $ \lambda _{coord} = 5,  \lambda_{noobj} = 0.5 $
        ...
        \lambda _{coord} \sum _{i=0}^{S^2} \sum _{j=0}^{B} \mathbb{1}_{ij}^{obj}(x_{i} - \hat x_{i})^{2} + (y_{i} - \hat y_{i})^{2} \\
        
        +  \lambda _{coord} \sum _{i=0}^{S^2} \sum _{j=0}^{B} \mathbb{1}_{ij}^{obj} (\sqrt{ w_{i}} - \sqrt{\hat w_{i}})^{2} + (\sqrt{h_{i}} - \sqrt{\hat y_{i}})^{2} \\
        
        +  \sum _{i=0}^{S^2} \sum _{j=0}^{B} \mathbb{1}_{ij}^{obj}(C_{i} - \hat C_{i})^{2} \\
        
        +  \lambda _{noobj} \sum _{i=0}^{S^2} \sum _{j=0}^{B} \mathbb{1}_{ij}^{obj}(C_{i} - \hat C_{i})^{2} \\
        
        +  \sum_{i=0}^{S^2} \mathbb{1}_{i}^{obj} \sum _{c \in classes} (p_{i}(c) - \hat p_{i}(c))^{2}
    ...
    All the entities in the image are represented as individual nodes on the graph
...
\section{Limitations}
    \begin{itemize}
        \item Since the images used in the pipeline are 2D and have no depth information, it is not possible to compute the actual distance between any two entities accurately.
    \end{itemize}
